...
inline auto dispatch_typed(const simdjson::dom::array        array,
  ...
  case simdjson::dom::element_type::INT64: {
    if (R_Type == rcpp_T::i32) {
      return has_nulls ? build_matrix_typed<INTSXP, int64_t, rcpp_T::i32, HAS_NULLS>(array, n_cols)
                       : build_matrix_typed<INTSXP, int64_t, rcpp_T::i32, NO_NULLS>(array, n_cols);
    }

    if constexpr (int64_opt == utils::Int64_R_Type::Double) {
      return has_nulls ? build_matrix_typed<REALSXP, int64_t, rcpp_T::dbl, HAS_NULLS>(array, n_cols)
                       : build_matrix_typed<REALSXP, int64_t, rcpp_T::dbl, NO_NULLS>(array, n_cols);
    }

    if constexpr (int64_opt == utils::Int64_R_Type::String) {
      return has_nulls ? build_matrix_typed<STRSXP, int64_t, rcpp_T::chr, HAS_NULLS>(array, n_cols)
